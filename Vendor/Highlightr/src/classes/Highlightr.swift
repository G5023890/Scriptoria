...
{
    private static let swiftPackageBundleName = "Highlightr_Highlightr.bundle"
    ...
    public init?(highlightPath: String? = nil)
    {
        ...
        #if SWIFT_PACKAGE
        guard let bundle = Self.resolveSwiftPackageBundle() else { return nil }
        #else
    ...
    #if SWIFT_PACKAGE
    private static func resolveSwiftPackageBundle() -> Bundle?
    {
        let candidateURLs = [
            Bundle.main.bundleURL.appendingPathComponent(swiftPackageBundleName, isDirectory: true),
            Bundle.main.resourceURL?.appendingPathComponent(swiftPackageBundleName, isDirectory: true),
